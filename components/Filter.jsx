...
function Filter({ setActiveStack, activeStack, setFiltered, allProjects }) {
  ...
  const Link = ({ text, activeStack, setActiveStack }) => {
    return (
      <button
        ...
        className={`${
          activeStack == text
            ? "group-hover:text-gray-400 text-main bg-foreground dark:bg-foreground-dark" : ""} relative inline-flex items-center justify-start inline-block px-5 py-3 overflow-hidden font-bold rounded-full group
        `}
      >
  ...
  return (
    <div className="my-6 flex justify-center gap-2 font-[Cormorant] overflow-x-scroll">
      <Link
        text="All"
